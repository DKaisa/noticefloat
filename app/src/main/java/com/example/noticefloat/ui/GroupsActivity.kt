...
class GroupsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        b.list.setOnItemLongClickListener { _, _, position, _ ->
            val (code, name, _) = items[position]
            AlertDialog.Builder(this)
                .setTitle("退出「$name」？")
                .setPositiveButton("退出") { _, _ -> leaveGroup(code) }
    ...
    private fun refresh() {
        lifecycleScope.launch {
            val result = withContext(Dispatchers.IO) {
                runCatching { api.myGroups() }
            }
            result.onSuccess { list ->
                items.clear(); items.addAll(list)
                val labels = list.map { "${it.second}  ·  #${it.first}  ·  ${it.third} 人" }
    ...
    private fun showJoinDialog() {
        val dialogB = DialogInputBinding.inflate(layoutInflater)
        dialogB.tvHint.text = "输入 8 位群号"
        dialogB.etInput.hint = "例：12345678"
        AlertDialog.Builder(this)
            .setTitle("加入群")
            .setView(dialogB.root)
            .setPositiveButton("加入") { _, _ ->
                val code = dialogB.etInput.text?.toString()?.trim().orEmpty()
                    .replace(Regex("[^0-9]"), "")
                if (code.length != 8) {
                    Toast.makeText(this, "群号应为 8 位数字", Toast.LENGTH_SHORT).show()
                    return@setPositiveButton
                }
                lifecycleScope.launch {
                    val r = withContext(Dispatchers.IO) { runCatching { api.joinGroup(code) } }
                    r.onSuccess { name ->
                        Toast.makeText(this@GroupsActivity, "已加入「$name」", Toast.LENGTH_LONG).show()
                        refresh()
                    }.onFailure {
